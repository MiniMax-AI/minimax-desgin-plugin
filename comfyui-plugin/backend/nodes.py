...
# Hub customization: only expose the selected partner nodes routed through the
# design gateway. All other partner-category nodes are unregistered.
HUB_PARTNER_NODE_ALLOWLIST = {
    "Banana2Node",
    "BananaProNode",
    "GImage2Node",
...
def _is_partner_category(category: str) -> bool:
    return category == "partner" or category.startswith("partner/")


def hub_hide_partner_nodes():
    hidden = []
    for name in list(NODE_CLASS_MAPPINGS.keys()):
        if name in HUB_PARTNER_NODE_ALLOWLIST:
            continue
        node_cls = NODE_CLASS_MAPPINGS[name]
        try:
            category = getattr(node_cls, "CATEGORY", "") or ""
        except Exception:
            category = ""
        if _is_partner_category(category):
            NODE_CLASS_MAPPINGS.pop(name, None)
            NODE_DISPLAY_NAME_MAPPINGS.pop(name, None)
            hidden.append(name)
    if hidden:
        logging.info(
            "[hub] hid %d partner node(s), keeping only %s",
            len(hidden),
            sorted(HUB_PARTNER_NODE_ALLOWLIST),
        )
    return hidden
...
async def init_extra_nodes(init_custom_nodes=True, init_api_nodes=True):
    ...
    # Hub customization: hide all partner nodes except the allowlisted one. Run
    # after every node source is loaded so it also covers any partner-category
    # nodes contributed by custom nodes.
    hub_hide_partner_nodes()
