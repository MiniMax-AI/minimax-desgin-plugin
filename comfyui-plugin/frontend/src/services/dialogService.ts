...
export type SaveBeforeCloseDecision = 'save' | 'saveAs' | 'discard' | null
...
type SaveBeforeCloseConfirmOptions = BaseConfirmOptions & {
...
export const useDialogService = () => {
  ...
  async function promptWithSecondary({
    title,
    message,
    defaultValue = '',
    placeholder,
  ...
  }): Promise<{ value: string; secondaryValue: string } | null> {
    return new Promise((resolve) => {
      dialogStore.showDialog({
        key: 'global-prompt',
        title,
        ...
        props: {
          message,
          defaultValue,
          onConfirm: (value: string, secondaryValue: string = '') => {
            resolve({ value, secondaryValue })
          },
          placeholder,
          secondaryMessage,
          secondaryDefaultValue,
  ...
  /**
   * Confirms a workflow close: callers can save, save under a new name, or
   * discard edits. It is used by both the workflow tab and Hub fullscreen.
   */
  async function confirmSaveBeforeClose({
    ...
    itemList = [],
    hint
  }: SaveBeforeCloseConfirmOptions): Promise<SaveBeforeCloseDecision> {
    return new Promise((resolve) => {
      const options: ShowDialogOptions = {
        key: 'save-before-close',
        title,
        component: ConfirmationDialogContent,
        props: {
          message,
