...
class NodeHelpService {
  private hubBaseDirectoryRequest: Promise<string | null> | null = null
  ...
  private async fetchCoreNodeHelp(
    ...
    this.hubBaseDirectoryRequest ??= fetchHubComfyBaseDirectory()
    const baseDirectory = await this.hubBaseDirectoryRequest
    return baseDirectory
      ? replaceComfyModelPaths(doc.text, baseDirectory)
      : doc.text
  }
