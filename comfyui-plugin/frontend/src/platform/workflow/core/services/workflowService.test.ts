...
const {
  mockConfirm,
  mockConfirmSaveBeforeClose,
  mockHubBackendOrigin,
  mockPrompt,
  mockPromptWithSecondary,
  mockTrackWorkflowSaved
} = vi.hoisted(() => ({
  mockConfirm: vi.fn(),
  mockConfirmSaveBeforeClose: vi.fn(),
  mockHubBackendOrigin: vi.fn<() => string | null>(() => null),
  mockPrompt: vi.fn(),
  mockPromptWithSecondary: vi.fn(),
...
vi.mock('@/services/dialogService', () => ({
  useDialogService: () => ({
    prompt: mockPrompt,
    promptWithSecondary: mockPromptWithSecondary,
    confirm: mockConfirm,
    confirmSaveBeforeClose: mockConfirmSaveBeforeClose
  })
}))

vi.mock('@/hub/backend-origin', () => ({
  hubBackendOrigin: mockHubBackendOrigin
}))
...
describe('useWorkflowService', () => {
  beforeEach(() => {
    ...
    vi.clearAllMocks()
    mockHubBackendOrigin.mockReturnValue(null)
  ...
  describe('saveWorkflow', () => {
    ...
    it('builds an executable snapshot after saving in Hub', async () => {
      mockHubBackendOrigin.mockReturnValue('http://127.0.0.1:8188')
      const workflow = createModeTestWorkflow({
        path: 'workflows/persisted.json'
      })
      vi.mocked(workflowStore.saveWorkflow).mockResolvedValue()
    ...
    it('does not rebind the Hub canvas node when saving in place', async () => {
      mockHubBackendOrigin.mockReturnValue('http://127.0.0.1:8188')
      const updateNodeData = vi.fn()
      Object.defineProperty(window, 'hub', {
        configurable: true,
        value: {
      ...
      Reflect.deleteProperty(window, 'hub')
    })
  ...
  describe('closeWorkflow', () => {
    ...
    it('keeps a temporary workflow open when Save As is cancelled', async () => {
      ...
      Object.defineProperty(workflow, 'isTemporary', { get: () => true })
      vi.spyOn(workflow, 'promptSave').mockResolvedValue(null)
      mockConfirmSaveBeforeClose.mockResolvedValue('save')
    ...
    it('keeps a persisted workflow open when Save As is cancelled', async () => {
      const workflow = createModeTestWorkflow({
        path: 'workflows/edited.json'
      })
      workflow.isModified = true
      vi.spyOn(workflow, 'promptSave').mockResolvedValue(null)
      mockConfirmSaveBeforeClose.mockResolvedValue('saveAs')
  ...
  describe('afterLoadNewGraph', () => {
    ...
    it('keeps only a fresh empty workflow for a Hub null-path load', async () => {
      mockHubBackendOrigin.mockReturnValue('http://127.0.0.1:18188')
      vi.mocked(workflowStore.getWorkflowByPath).mockReturnValue(null)
      const emptyWorkflow = createModeTestWorkflow({
        path: 'workflows/Unsaved Workflow.json'
      })
      vi.mocked(workflowStore.createNewTemporary).mockReturnValue(emptyWorkflow)
      vi.mocked(workflowStore.openWorkflow).mockResolvedValue(emptyWorkflow)

      await useWorkflowService().afterLoadNewGraph(null, {
        nodes: [],
        links: []
      } as never)
      ...
      expect(workflowStore.keepOnlyWorkflow).toHaveBeenCalledWith(emptyWorkflow)
    })
  ...
  describe('saveWorkflowAs', () => {
    ...
    it('captures Hub workflow purpose while prompting for a filename', async () => {
      mockHubBackendOrigin.mockReturnValue('http://127.0.0.1:8188')
      mockPromptWithSecondary.mockResolvedValue({
        value: 'product-workflow',
        secondaryValue: 'Generate product photos'
      })
      const workflow = createTemporaryWorkflow()
      workflow.changeTracker = createMockChangeTracker({
        activeState: makeWorkflowData()
      })

      const result = await service.saveWorkflowAs(workflow)

      expect(result).toBe(true)
      expect(mockPromptWithSecondary).toHaveBeenCalledWith(
        expect.objectContaining({
      ...
      expect(workflow.activeState.extra).toMatchObject({
        hub: {
          schema_version: 1,
          agent_hint: 'Generate product photos'
        }
      })
      expect(app.rootGraph.extra).toMatchObject({
        hub: {
          schema_version: 1,
          agent_hint: 'Generate product photos'
        }
    ...
    it('does not ask for workflow purpose again after it has been saved', async () => {
      mockHubBackendOrigin.mockReturnValue('http://127.0.0.1:8188')
      const workflow = createTemporaryWorkflow()
      workflow.changeTracker = createMockChangeTracker({
        activeState: makeWorkflowData({
          hub: { schema_version: 1, agent_hint: '' }
        })
      })
      vi.spyOn(workflow, 'promptSave').mockResolvedValue('existing-workflow')

      await service.saveWorkflowAs(workflow)

      expect(workflow.promptSave).toHaveBeenCalledOnce()
      expect(mockPromptWithSecondary).not.toHaveBeenCalled()
    })
