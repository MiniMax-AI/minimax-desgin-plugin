...
interface WorkflowStore {
  ...
  /** Keep only one workflow open for the embedded Hub editor. */
  keepOnlyWorkflow: (workflow?: ComfyWorkflow | null) => Promise<void>
  createTemporary: (
    path?: string,
...
export const useWorkflowStore = defineStore('workflow', () => {
  ...
  const saveAs = (
    ...
    // Hub may restore a workflow from serialized draft data. Such a source can
    // lose its ComfyWorkflow prototype, so deriving the copy constructor from
    // the source would create a plain object without save(). Always create the
    // concrete workflow type expected by saveWorkflow instead.
  ...
  const closeWorkflow = async (workflow: ComfyWorkflow) => {
    ...
    if (!window.hub) useWorkflowDraftStoreV2().removeDraft(workflow.path)
  ...
  const keepOnlyWorkflow = async (workflow?: ComfyWorkflow | null) => {
    const retained = workflow ?? activeWorkflow.value
    if (!retained) return

    const otherWorkflows = openWorkflows.value.filter(
      (candidate) => candidate?.path !== retained.path
    )
    for (const other of otherWorkflows) {
      await closeWorkflow(other)
    }

    openWorkflowPaths.value = [retained.path]
    tabActivationHistory.value = [retained.path]
  }
  ...
  const {
    ...
    async (dir: string = '') => {
      await syncEntities(
        ...
        /* include */ (file) => file.path.toLowerCase().endsWith('.json')
      )
    },
    undefined,
    { immediate: false }
  ...
  const saveWorkflow = async (workflow: ComfyWorkflow) => {
    // saveAs() constructs a new temporary file with serialized content. In
    // Hub that copy stays inactive, so it does not pass through openWorkflow
    // and would otherwise have no ChangeTracker/activeState to persist.
    if (!workflow.activeState && workflow.content !== null) {
      await workflow.load()
    }
    if (!workflow.activeState) {
      throw new Error(`Cannot save workflow '${workflow.path}': state is unavailable`)
    }
    isBusy.value = true
    try {
      await workflow.save()
