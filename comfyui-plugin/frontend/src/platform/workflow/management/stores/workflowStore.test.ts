...
import {
  ComfyWorkflow as ComfyWorkflowClass,
...
describe('useWorkflowStore', () => {
  ...
  describe('syncWorkflows', () => {
    ...
    it('should only sync JSON workflow files', async () => {
      await syncRemoteWorkflows(['a.json', '.index', 'notes.txt', 'b.JSON'])

      expect(store.workflows.map((workflow) => workflow.path)).toEqual([
        'workflows/a.json',
        'workflows/b.JSON'
      ])
    })
  ...
  describe('keepOnlyWorkflow', () => {
    it('closes all background workflows and keeps the active workflow', async () => {
      const workflowA = store.createNewTemporary('hub-a.json', defaultGraph)
      const workflowB = store.createNewTemporary('hub-b.json', defaultGraph)

      await store.openWorkflow(workflowA)
      store.openWorkflowsInBackground({ right: [workflowB.path] })

      await store.keepOnlyWorkflow(workflowA)

      expect(store.openWorkflows.map((workflow) => workflow.path)).toEqual([
        workflowA.path
      ])
      expect(store.activeWorkflow?.path).toBe(workflowA.path)
      expect(store.getWorkflowByPath(workflowB.path)).toBeNull()
    })
  })
  ...
  describe('save', () => {
    it('initializes a copied workflow before saving it', async () => {
      const source = store.createTemporary('source.json', {
        ...JSON.parse(defaultGraphJSON),
        id: 'source-id'
      })
      await store.openWorkflow(source)
      const copy = store.saveAs(source, 'workflows/copy.json')
      vi.mocked(api.storeUserData).mockResolvedValue({
        status: 200,
        json: () =>
          Promise.resolve({
      ...
      await store.saveWorkflow(copy)

      expect(copy.activeState).toMatchObject({ id: expect.any(String) })
      expect(copy.activeState?.id).not.toBe('source-id')
      expect(api.storeUserData).toHaveBeenCalledWith(
        'workflows/copy.json',
        JSON.stringify(copy.activeState),
  ...
  describe('saveAs', () => {
    it('creates a savable ComfyWorkflow copy when a Hub source lost its prototype', () => {
      const source = {
        path: 'workflows/source.json',
        content: defaultGraphJSON,
        activeState: JSON.parse(defaultGraphJSON),
        initialMode: 'graph'
      } as unknown as ComfyWorkflow

      const copy = store.saveAs(source, 'workflows/copied.json')

      expect(copy).toBeInstanceOf(ComfyWorkflowClass)
      expect(copy.save).toBeTypeOf('function')
    })
