...
export function useWorkflowPersistenceV2() {
  ...
  const resolveStartupOutcome = async (): Promise<StartupOutcome> => {
    // A Hub iframe is a fresh editor for one canvas node. Do not let
    // ComfyUI's standalone startup restore its global default workflow here;
    // hub-bootstrap will load the node workflow (or the node draft) next.
    if (hubBackendOrigin() !== null) {
      await comfyApp.loadGraphData(blankGraph)
      return 'fresh'
    }
  ...
  const initializeWorkflow = async (): Promise<StartupOutcome> => {
    // Hub embeds one canvas per ComfyUI node. Never restore the user's global
    // tab session here: the host will provide the workflow for this node.
    if (hubBackendOrigin() !== null) {
  ...
  const loadSharedWorkflowFromUrlIfPresent = async () => {
    // Cloud workflow ownership/sharing is disabled in local mode.
    return undefined
  }
  ...
  const restoreWorkflowTabsState = async () => {
    if (hubBackendOrigin() !== null) {
      ...
      await workflowStore.keepOnlyWorkflow(workflowStore.activeWorkflow)
      return
    }
    if (!workflowPersistenceEnabled.value) {
      tabStateRestored = true
