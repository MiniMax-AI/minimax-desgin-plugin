...
export const useTeamWorkspaceStore = defineStore('teamWorkspace', () => {
  ...
  async function initialize(): Promise<void> {
    // Team workspaces are not available in local mode. Keep the store ready
    // with an empty state and never create a session or call the cloud API.
  ...
  async function refreshWorkspaces(): Promise<void> {
    // No remote workspace list in local mode.
    workspaces.value = []
    activeWorkspaceId.value = null
  }
