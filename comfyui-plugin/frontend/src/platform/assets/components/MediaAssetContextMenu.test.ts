...
function mountComponent(
  ...
  const { container, unmount } = render(
    defineComponent({
      ...
      template:
        '<MediaAssetContextMenu ref="menuRef" :asset="asset" asset-type="output" file-kind="image" :show-workflow-actions="showWorkflowActions" :selected-assets="selectedAssets" :is-bulk-mode="isBulkMode" @hide="onHide" />'
    }),
    {
      global: {
        stubs: {
...
describe('MediaAssetContextMenu', () => {
  ...
  it('hides workflow export when the caller disables workflow actions', async () => {
    const { container, unmount } = mountComponent(asset, false)
    await showMenu(container)

    expect(findMenuItem('mediaAsset.actions.openWorkflow')).toBeUndefined()
    expect(findMenuItem('mediaAsset.actions.exportWorkflow')).toBeUndefined()

    unmount()
  })

  it('hides bulk workflow export when the caller disables workflow actions', async () => {
    const { container, unmount } = mountComponent(asset, false, [asset], true)
    await showMenu(container)

    expect(findMenuItem('mediaAsset.selection.openWorkflowAll')).toBeUndefined()
    expect(
      findMenuItem('mediaAsset.selection.exportWorkflowAll')
    ).toBeUndefined()

    unmount()
  })

  it('never shows open workflow actions', async () => {
    const individual = mountComponent()
    await showMenu(individual.container)

    expect(findMenuItem('mediaAsset.actions.openWorkflow')).toBeUndefined()
    expect(findMenuItem('mediaAsset.actions.exportWorkflow')).toBeDefined()

    individual.unmount()

    const bulk = mountComponent(asset, true, [asset], true)
    await showMenu(bulk.container)

    expect(findMenuItem('mediaAsset.selection.openWorkflowAll')).toBeUndefined()
    expect(findMenuItem('mediaAsset.selection.exportWorkflowAll')).toBeDefined()
