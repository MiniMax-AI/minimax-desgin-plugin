...
const mockGetIncomingResource = vi.hoisted(() => vi.fn())
const mockGetWorkspaceResource = vi.hoisted(() => vi.fn())
const mockImportIncomingResourceWithToast = vi.hoisted(() => vi.fn())
const mockImportWorkspaceResourceWithToast = vi.hoisted(() => vi.fn())
vi.mock('@/services/incomingAssetService', () => ({
  getIncomingResource: mockGetIncomingResource,
  getWorkspaceResource: mockGetWorkspaceResource,
  importIncomingResourceWithToast: mockImportIncomingResourceWithToast,
  importWorkspaceResourceWithToast: mockImportWorkspaceResourceWithToast
}))
...
function createLoadImageNode(): LGraphNode {
  return fromAny<LGraphNode, unknown>({
    widgets: [
      { name: 'image', value: '', options: { values: [] }, callback: vi.fn() }
    ],
...
function getAddedImageWidgetOptions() {
  return litegraphServiceMock.addNodeOnGraph.mock.results.map(
    ({ value }) =>
      value.widgets?.find((widget: IWidget) => widget.name === 'image')?.options
        .values
  )
}
...
describe('useMediaAssetActions', () => {
  beforeEach(() => {
    ...
    mockGetIncomingResource.mockReset()
    mockGetWorkspaceResource.mockReset()
    mockImportIncomingResourceWithToast.mockReset()
    mockImportWorkspaceResourceWithToast.mockReset()
  })
  ...
  describe('addWorkflow', () => {
    describe('OSS mode (isCloud = false)', () => {
      ...
      it('imports an incoming resource before adding its loader node', async () => {
        const incomingResource = {
          ...
          path: '/outputs/upstream.png'
        }
        mockGetIncomingResource.mockReturnValue(incomingResource)
        mockImportIncomingResourceWithToast.mockResolvedValue(
          'hub-upstream/upstream.png'
        )
        const actions = useMediaAssetActions()

        await actions.addWorkflow(
          createMockAsset({
            id: 'upstream-node:upstream-asset',
            name: 'upstream.png'
          })
        )

        expect(mockGetIncomingResource).toHaveBeenCalledWith(
          'upstream-node:upstream-asset'
        )
        expect(mockImportIncomingResourceWithToast).toHaveBeenCalledWith(
          incomingResource
        )
        expect(getAddedImageWidgetValues()).toEqual([
          'hub-upstream/upstream.png'
        ])
        expect(getAddedImageWidgetOptions()).toEqual([
          ['hub-upstream/upstream.png']
        ])
        expect(mockUpdateInputs).toHaveBeenCalledOnce()
      })

      it('imports a Design-canvas resource before adding its loader node', async () => {
        const workspaceResource = {
          nodeId: 'design-node',
          assetId: 'design-asset',
          type: 'image' as const,
          name: 'design.png',
          url: 'http://localhost/design.png',
          path: '/workspace/design.png'
        }
        mockGetWorkspaceResource.mockReturnValue(workspaceResource)
        mockImportWorkspaceResourceWithToast.mockResolvedValue(
          'hub-upstream/design.png'
        )
        const actions = useMediaAssetActions()

        await actions.addWorkflow(
          createMockAsset({
            id: 'design-node:design-asset',
            name: 'design.png'
          })
        )

        expect(mockGetIncomingResource).toHaveBeenCalledWith(
          'design-node:design-asset'
        )
        expect(mockGetWorkspaceResource).toHaveBeenCalledWith(
          'design-node:design-asset'
        )
        expect(mockImportWorkspaceResourceWithToast).toHaveBeenCalledWith(
          workspaceResource
        )
        expect(getAddedImageWidgetValues()).toEqual(['hub-upstream/design.png'])
        expect(getAddedImageWidgetOptions()).toEqual([
          ['hub-upstream/design.png']
        ])
        expect(mockUpdateInputs).toHaveBeenCalledOnce()
