  ...
  onMounted(() => {
    ...
    extensionStore.registerExtension({
      ...
      nodeCreated(node: LGraphNode) {
        ...
        const hubBillingNodeType = resolveHubBillingNodeType(
          node.type,
          node.constructor.nodeData?.name
        )
        if (
          (hubBillingNodeType || node.isSubgraphNode()) &&
          showApiPricingBadge.value
        ) {
          const priceDescription = hubBillingNodeType
            ? useHubNodePriceDescription(hubBillingNodeType)
            : computed(() => getHubSubgraphPriceDescription(node))
          const priceInfoBadge = new LGraphBadge({
            text: '',
            fontSize: 12,
            height: 20,
            padding: 7,
          ...
          node.badges.push(() => {
            const appearance = getHubPriceBadgeAppearance(
              priceDescription.value,
              st('nodeHelpPage.priceDescription', 'Price details'),
              colorPaletteStore.completedActivePalette.colors.litegraph_base
            )
            priceInfoBadge.text = appearance.text
            priceInfoBadge.tooltip = formatHubNodePriceDescriptionForTooltip(
              priceDescription.value
            )
            priceInfoBadge.fgColor = appearance.fgColor
            priceInfoBadge.bgColor = appearance.bgColor
            return priceInfoBadge
          })
          watch(priceDescription, () => app.canvas?.setDirty(true, true))
        }
