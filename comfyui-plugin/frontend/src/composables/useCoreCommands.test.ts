...
describe('useCoreCommands', () => {
  ...
  describe('Hub workflow saves', () => {
    it('delegates Cmd+S for a bound Hub workflow to the topbar', async () => {
      const handleUpdateSave = vi.fn()
      Object.defineProperty(globalThis, '__COMFY_BACKEND_ORIGIN__', {
        configurable: true,
        value: 'http://127.0.0.1:8188'
      })
      Object.defineProperty(window, '__COMFY_HUB_WORKFLOW_ID__', {
        configurable: true,
        value: 'template:workflow-a'
      })
      ...
      try {
        const saveCommand = useCoreCommands().find(
          (command) => command.id === 'Comfy.SaveWorkflow'
        )!
        await saveCommand.function()

        expect(handleUpdateSave).toHaveBeenCalledOnce()
      } finally {
        window.removeEventListener(
          'hub-comfyui:request-update-save',
          handleUpdateSave
        )
        delete (globalThis as { __COMFY_BACKEND_ORIGIN__?: string })
          .__COMFY_BACKEND_ORIGIN__
        delete (window as { __COMFY_HUB_WORKFLOW_ID__?: string })
