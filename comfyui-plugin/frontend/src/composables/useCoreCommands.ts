...
export function useCoreCommands(): ComfyCommand[] {
  ...
  const commands = [
    ...
    },
    {
      ...
      function: async () => {
        ...
        if (
          ...
          hubWorkflowId !== undefined &&
          hubWorkflowId !== 'empty'
        ) {
          window.dispatchEvent(
            new CustomEvent('hub-comfyui:request-update-save')
          )
          return
        }
    ...
    {
      id: 'Hub.EditWorkflowAgentHint',
      icon: 'pi pi-pencil',
      label: () => t('workflowService.editAgentHint'),
      menubarLabel: () => t('workflowService.editAgentHint'),
      function: async () => {
        const workflow = workflowStore.activeWorkflow
        if (!workflow) return
        if (!(await workflowService.editHubWorkflowAgentHint(workflow))) return
        await workflowService.saveWorkflow(workflow)
      }
    },
    {
