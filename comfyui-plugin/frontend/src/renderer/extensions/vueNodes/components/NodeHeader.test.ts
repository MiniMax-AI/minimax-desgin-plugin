...
describe('NodeHeader.vue', () => {
  ...
  describe('Tooltips', () => {
    it('shows current Hub price rules from the node header tip', async () => {
      window.hub = {
      ...
      const { tooltipDirective } = renderHeader({
        nodeData: makeNodeData({ type: 'MinimaxHailuo03TextToVideoNode' })
      })
      const priceInfo = screen.getByTestId('node-price-info')
      const priceTooltipCall = tooltipDirective.mounted.mock.calls.find(
        ([element]) => element === priceInfo
      )

      expect(priceTooltipCall?.[1].value.value).toBe(
        '• 768P: 56 credits/second'
      )
    })
