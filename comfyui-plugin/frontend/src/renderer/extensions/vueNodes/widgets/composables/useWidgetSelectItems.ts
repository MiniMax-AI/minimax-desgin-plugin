...
function isIncomingResourceCompatible(
...
export function useWidgetSelectItems(options: UseWidgetSelectItemsOptions) {
  ...
  const { resources: incomingResources } = useIncomingResources()
  ...
  const filterOptions = computed<FilterOption[]>(() => {
    ...
    return [
      ...
      { name: t('sideToolbar.labels.incoming'), value: 'incoming' },
  ...
  const incomingItems = computed<FormDropdownItem[]>(() => {
    const kind = toValue(options.assetKind)
    const incomingLabel = t('sideToolbar.labels.incoming')
    return incomingResources.value
      .filter((resource) => isIncomingResourceCompatible(resource, kind))
      .map((resource) => ({
        id: `incoming-${resource.nodeId}-${resource.assetId}`,
        name: resource.name,
        label: `${resource.name} [${incomingLabel}]`,
        preview_url:
          resource.type === 'image' || resource.type === 'video'
            ? resource.url
            : '',
        incomingResourceId: `${resource.nodeId}:${resource.assetId}`
      }))
  })
