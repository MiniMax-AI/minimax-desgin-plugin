...
export function useWidgetSelectActions(options: UseWidgetSelectActionsOptions) {
  ...
  async function updateSelectedItems(selectedItems: Set<string>) {
    const id =
      selectedItems.size > 0 ? selectedItems.values().next().value : undefined
    const selectedItem =
      id == null
        ? undefined
        : dropdownItems.value.find((item) => item.id === id)

    if (selectedItem?.incomingResourceId) {
      const resource = getIncomingResource(selectedItem.incomingResourceId)
      if (!resource) return

      const importedPath = await importIncomingResourceWithToast(resource)
      if (!importedPath) return

      const values = toValue(options.widget).options?.values
      if (Array.isArray(values) && !values.includes(importedPath)) {
        values.push(importedPath)
      }
      modelValue.value = importedPath
      try {
