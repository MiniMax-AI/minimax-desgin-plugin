...
import { shouldReuseActiveWorkflow } from '@/platform/workflow/core/utils/workflowLoadTarget'
import {
  ComfyWorkflow,
  useWorkflowStore
...
import { applyHubNodePriceDescriptions } from '@/hub/node-price-descriptions'
...
export class ComfyApp {
  ...
  async getNodeDefs(): Promise<Record<string, ComfyNodeDefV1>> {
    ...
    const translatedDefs = _.mapValues(defs, (def) => translateNodeDef(def))
    return await applyHubNodePriceDescriptions(translatedDefs)
  }
  ...
  async loadGraphData(
    ...
    options: {
      ...
      /** Reuse the active tab instead of creating a new temporary tab. */
      reuseActiveWorkflow?: boolean
    } = {}
  ) {
    const {
    ...
    try {
      ...
      const reuseActiveWorkflow = shouldReuseActiveWorkflow(
        options.reuseActiveWorkflow,
        hubBackendOrigin() !== null
      )
      const workflowTarget =
        reuseActiveWorkflow && activeWorkflow ? activeWorkflow : workflow
        ...
        for (let i = 0; i < batchCount; i++) {
          ...
          try {
            ...
            if (res.prompt_id) {
              ...
              void registerComfyRunWithHub(res.prompt_id).catch((error) => {
                console.warn('Failed to register ComfyUI prompt with Hub', {
                  promptId: res.prompt_id,
                  error
                })
              })
