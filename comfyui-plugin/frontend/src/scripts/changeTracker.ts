...
export class ChangeTracker {
  ...
  captureCanvasState() {
    ...
    if (!ChangeTracker.graphEqual(this.activeState, currentState)) {
      ...
      if (
        ChangeTracker.isViewportTriggeredNodeSizeReflow(
          previousState,
          currentState
        )
      ) {
        this.activeState = currentState
        this.initialState = clone(currentState)
        return
      }
  ...
  static isViewportTriggeredNodeSizeReflow(
    previous: ComfyWorkflowJSON,
    current: ComfyWorkflowJSON
  ): boolean {
    if (_.isEqual(previous.extra?.ds, current.extra?.ds)) return false

    const previousNodes = new Map(previous.nodes.map((node) => [node.id, node]))
    const normalizedCurrent = clone(current)
    let resizedNodeCount = 0

    for (const node of normalizedCurrent.nodes) {
      const previousNode = previousNodes.get(node.id)
      if (!previousNode || _.isEqual(previousNode, node)) continue
      if (!_.isEqual(previousNode.size, node.size)) {
        node.size = previousNode.size
        resizedNodeCount++
      }
    }
    ...
    return (
      resizedNodeCount > 0 &&
      ChangeTracker.graphEqual(previous, normalizedCurrent)
    )
  }
}
