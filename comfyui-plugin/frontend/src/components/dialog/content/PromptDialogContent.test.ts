...
describe('PromptDialogContent', () => {
  ...
  it('submits the optional secondary value', async () => {
    const onConfirm = vi.fn()
    const { user } = renderComponent({
      defaultValue: 'workflow',
      onConfirm,
      secondaryMessage: 'Workflow purpose',
      secondaryDefaultValue: 'Portraits',
      secondaryMaxLength: 200
    })
    const [, secondaryInput] = screen.getAllByRole('textbox')

    await user.clear(secondaryInput)
    await user.type(secondaryInput, 'Product photos')
    await user.click(screen.getByRole('button', { name: /confirm/i }))

    expect(onConfirm).toHaveBeenCalledWith('workflow', 'Product photos')
