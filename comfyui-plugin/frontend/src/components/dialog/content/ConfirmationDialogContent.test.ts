...
describe('ConfirmationDialogContent', () => {
  ...
  describe('button surface per type', () => {
    ...
    it("type='saveBeforeClose' renders Cancel and Save", () => {
      renderComponent({ type: 'saveBeforeClose' })
      expect(screen.getByRole('button', { name: 'Cancel' })).toBeInTheDocument()
      expect(screen.getByRole('button', { name: 'Save' })).toBeInTheDocument()
    })
  ...
  describe('save before close', () => {
    it('returns discard when Cancel is clicked', async () => {
      const onConfirm = vi.fn()
      const { user } = renderComponent({
