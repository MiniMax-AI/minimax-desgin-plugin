<template>
  ...
  >
    ...
    <div class="flex shrink-0 flex-wrap justify-end gap-4">
      ...
      <template v-else-if="type === 'saveBeforeClose'">
        <Button variant="secondary" @click="onDeny">
          <i class="pi pi-times" />
          {{ $t('g.cancel') }}
        </Button>
...
const props = defineProps<{
  ...
  onConfirm: (value?: boolean | 'save' | 'saveAs' | 'discard') => void
  itemList?: string[]
  hint?: string
  denyLabel?: string
}>()
...
const onDeny = () => {
  props.onConfirm(props.type === 'saveBeforeClose' ? 'discard' : false)
  useDialogStore().closeDialog()
}
...
const onConfirm = () => {
  ...
  props.onConfirm(props.type === 'saveBeforeClose' ? 'save' : true)
  useDialogStore().closeDialog()
}
</script>
