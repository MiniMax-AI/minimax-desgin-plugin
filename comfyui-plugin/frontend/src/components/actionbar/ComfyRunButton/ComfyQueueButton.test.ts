...
function renderQueueButton(
  props: { paymentRecoveryLock?: 'owner' | 'member'; tooltip?: string } = {}
) {
...
describe('ComfyQueueButton', () => {
  ...
  it('uses the supplied tooltip when embedded in Hub', () => {
    renderQueueButton({ tooltip: 'Run directly with the current draft' })

    expect(screen.getByTestId('queue-button')).toHaveAttribute(
      'data-tooltip',
      'Run directly with the current draft'
    )
  })
