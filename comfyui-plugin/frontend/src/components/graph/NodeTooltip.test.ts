...
describe('NodeTooltip', () => {
  beforeEach(() => {
    ...
    mockCanvas.visible_nodes = [mockCanvas.node_over]
  ...
  it('shows a badge tooltip before falling back to the node title tooltip', async () => {
    mockCanvas.graph_mouse = [92, -12]
    const node: MockNode = {
      ...createSam3Node(),
      badges: [
        {
          visible: true,
          tooltip: '• 768P: 56 credits/second',
          boundingRect: [80, -24, 28, 20]
        }
      ]
    }
    mockCanvas.node_over = null
    mockCanvas.visible_nodes = [node]

    await renderAndHoverCanvas()

    const tooltip = screen.getByText(/768P/)
    expect(tooltip).not.toHaveTextContent('Price details')
    expect(tooltip).toHaveTextContent('768P: 56 credits/second')
  })
