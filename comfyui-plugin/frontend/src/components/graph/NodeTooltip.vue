...
function getBadgeTooltipAtCursor(): string | undefined {
  const { canvas } = comfyApp
  for (const node of [...(canvas.visible_nodes ?? [])].reverse()) {
    const nodeX = canvas.graph_mouse[0] - node.pos[0]
    const nodeY = canvas.graph_mouse[1] - node.pos[1]
    for (const badgeValue of node.badges) {
      const badge = typeof badgeValue === 'function' ? badgeValue() : badgeValue
      const [x, y, width, height] = badge.boundingRect
      if (
        badge.visible &&
        badge.tooltip &&
        nodeX >= x &&
...
function onIdle() {
  const { canvas } = comfyApp
  const badgeTooltip = getBadgeTooltipAtCursor()
  if (badgeTooltip) return showTooltip(badgeTooltip)
