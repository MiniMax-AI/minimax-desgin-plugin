<template>
  ...
  <LiteGraphCanvasSplitterOverlay v-if="comfyAppReady && !isHubGraphPreview">
  ...
  <SelectionRectangle
    v-if="comfyAppReady && !isHubGraphPreview"
    :panel-el="canvasPanelBoundsRef ?? undefined"
  />

  <template v-if="!isHubGraphPreview">
  ...
  <template v-if="comfyAppReady && !isHubGraphPreview">
    <TitleEditor />
...
import HubWorkflowTopbarActions from '@/hub/HubWorkflowTopbarActions.vue'
...
import { useHubFullscreenCloseGuard } from '@/hub/useHubFullscreenCloseGuard'
import {
  scheduleAfterCanvasLayout,
  useHubGraphPreview
} from '@/hub/useHubGraphPreview'
import { useHubWorkflowDraftPersistence } from '@/hub/useHubWorkflowDraftPersistence'
...
const { isHubGraphPreview } = useHubGraphPreview()
const showUI = computed(
  () =>
    !isHubGraphPreview.value &&
...
let previewFitRequest = 0

const schedulePreviewFitView = () => {
  if (!isHubGraphPreview.value || !comfyAppReady.value) return
  const request = ++previewFitRequest
  void nextTick(() => {
    scheduleAfterCanvasLayout(() => {
      if (
        request !== previewFitRequest ||
        !isHubGraphPreview.value ||
        !comfyAppReady.value
      ) {
        return
...
watch([isHubGraphPreview, comfyAppReady], ([isPreview, isReady]) => {
  if (isPreview && isReady) {
    schedulePreviewFitView()
  } else if (!isPreview) {
    comfyApp.canvas?.setDirty(true, true)
  }
})

const handleWorkflowReadyForPreview = () => schedulePreviewFitView()
useResizeObserver(canvasRef, () => schedulePreviewFitView())
const workflowPersistence = useWorkflowPersistence()
const { runUrlActionLoaders } = useUrlActionLoaders()
useCanvasDrop(canvasRef)
useLitegraphSettings()
useNodeBadge()
useHubFullscreenCloseGuard()
useHubWorkflowDraftPersistence()
...
let hubFullscreenDisposer: (() => void) | null = null
let graphCanvasMounted = false
const HUB_BRIDGE_WAIT_INTERVAL_MS = 50
const HUB_BRIDGE_WAIT_TIMEOUT_MS = 10_000

const waitForHubBridge = async () => {
  const deadline = Date.now() + HUB_BRIDGE_WAIT_TIMEOUT_MS
  while (graphCanvasMounted && !window.hub && Date.now() < deadline) {
...
const setupHubFullscreenGuard = async () => {
  const hub = await waitForHubBridge()
  if (!hub) return
  try {
    await hub.ready
  } catch {
  ...
  if (!graphCanvasMounted || hubFullscreenDisposer) return
  const handleFullscreenChange = (fullscreen: boolean) => {
    if (fullscreen) {
      void startIncomingResourceSync()
    } else {
      stopIncomingResourceSync()
      stopWorkspaceResourceSync()
    }
  }
  hubFullscreenDisposer =
    hub.ui?.onFullscreenChange?.(handleFullscreenChange) ?? null
  // The launcher iframe is created as fullscreen opens. Its initial cached
  // fullscreen flag can still be false when `hub.ready` resolves, and the
  // corresponding host event may already have raced with this subscription.
  // Always seed the first snapshot; later fullscreen events still own
  // stopping and restarting the sync.
  void startIncomingResourceSync()
}

const closeEmbeddedWorkflows = async () => {
  // A ComfyUI iframe belongs to exactly one canvas node. Once that iframe is
  // being destroyed, release every tab it owns so a later iframe cannot
  // inherit stale workflow tabs from this node.
  const workflows = [...workflowStore.openWorkflows]
  await Promise.all(
    workflows.map((workflow) => workflowStore.closeWorkflow(workflow))
  )
}
...
onMounted(async () => {
  graphCanvasMounted = true
  window.addEventListener(
    'hub-comfyui:workflow-ready',
    handleWorkflowReadyForPreview
  )
  void setupHubFullscreenGuard()
...
onUnmounted(() => {
  ...
  hubFullscreenDisposer?.()
  hubFullscreenDisposer = null
  stopIncomingResourceSync()
  stopWorkspaceResourceSync()
  void closeEmbeddedWorkflows()
  cleanupErrorHooks?.()
