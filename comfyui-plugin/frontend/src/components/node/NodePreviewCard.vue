<template>
  ...
  >
    ...
    <div class="flex flex-col gap-2 p-3 pt-1">
      ...
      <div
        v-if="renderedPriceDescription"
        class="[&_h3]:text-foreground border-t border-border-default pt-2 text-2xs/normal text-muted-foreground [&_h3]:m-0 [&_h3]:mb-1 [&_h3]:text-2xs [&_h3]:font-semibold [&_li]:my-0.5 [&_ul]:m-0 [&_ul]:pl-4"
        data-testid="node-preview-price-description"
        v-html="renderedPriceDescription"
      />
...
import { useHubNodePriceDescription } from '@/hub/node-price-descriptions'
...
const priceDescription = useHubNodePriceDescription(() => nodeDef.name)
const renderedPriceDescription = computed(() =>
  renderMarkdownToHtml(priceDescription.value)
)
