...
function renderDropdown(showAppModeSwitch = true) {
  const user = userEvent.setup()
  const result = render(WorkflowActionsDropdown, {
    props: { source: 'test', showAppModeSwitch },
    global: {
      plugins: [i18n],
      directives: { tooltip: {} },
      stubs: {
...
describe('WorkflowActionsDropdown', () => {
  ...
  it('can hide the app mode switch while keeping workflow actions available', () => {
    renderDropdown(false)

    expect(
      screen.queryByRole('button', { name: 'Enter app mode' })
