...
const {
  source,
  align = 'start',
  showAppModeSwitch = true
} = defineProps<{
  source: string
  align?: 'start' | 'center' | 'end'
  showAppModeSwitch?: boolean
}>()
...
const orderedSegments = computed(() => {
  const visibleSegments = showAppModeSwitch
    ? segments.value
    : segments.value.filter((seg) => seg.mode === appModeStore.viewMode)
  if (visibleSegments.length < 2) return visibleSegments

  const [graph, app] = visibleSegments
