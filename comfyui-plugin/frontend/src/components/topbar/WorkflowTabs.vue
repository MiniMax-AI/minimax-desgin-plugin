<template>
  <div
    v-if="isHubPlugin"
    class="workflow-tabs-container flex h-full min-w-0 flex-1 items-center justify-start overflow-hidden"
  >
    <span class="whitespace-nowrap p-2 text-sm text-base-foreground">
  ...
  >
    <Button
      v-if="!isHubPlugin && showOverflowArrows"
      ...
      >
        <template #option="{ option, index }">
          <WorkflowTab
            ...
            @click.middle="isHubPlugin ? undefined : onCloseWorkflow(option)"
...
const options = computed<WorkflowOption[]>(() => {
  if (isHubPlugin) {
    return workflowStore.activeWorkflow
      ? [workflowToOption(workflowStore.activeWorkflow)]
      : []
  }
  return workflowStore.openWorkflows.map(workflowToOption)
})
