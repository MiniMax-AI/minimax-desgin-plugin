  ...
  return render(AssetsSidebarTab, {
    global: {
      ...
      stubs: {
        ...
        IncomingResourcesPanel: true,
        Button: buttonStub,
        MediaAssetFilterBar: true,
        MediaAssetSettingsButton: {
          template: '<button aria-label="View settings"><slot /></button>'
        },
        MediaAssetSettingsMenu: true,
...
describe('AssetsSidebarTab folder navigation', () => {
  it('places view settings beside the media assets title', () => {
    renderTab()

    expect(
      within(screen.getByTestId('folder-title')).getByRole('button', {
        name: 'View settings'
      })
    ).toBeVisible()
  })
