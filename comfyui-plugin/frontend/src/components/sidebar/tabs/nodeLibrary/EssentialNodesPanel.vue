...
// A tile is shown only if its backing node can actually be created. Blueprint
// tiles are always kept (they resolve lazily once blueprints load); other tiles
// must resolve to a registered node def. This automatically hides partner nodes
// that the backend chooses not to register (see hub_hide_partner_nodes).
const isTileAvailable = (tile: EssentialTile) => {
  if (tile.nodeName.startsWith(BLUEPRINT_TYPE_PREFIX)) return true
  return resolveEssentialTileNodeDef(tile, nodeDefStore) != null
}
...
const filteredSections = computed<EssentialSection[]>(() => {
  ...
  const keepTile = (tile: EssentialTile) =>
    isTileAvailable(tile) && matchesQuery(tile)

  return ESSENTIAL_SECTIONS.flatMap<EssentialSection>((section) => {
    if (section.tiles?.length) {
      const tiles = section.tiles.filter(keepTile)
      return tiles.length ? [{ ...section, tiles }] : []
    }

    const subgroups = section.subgroups
      ?.map((sg) => ({ ...sg, tiles: sg.tiles.filter(keepTile) }))
